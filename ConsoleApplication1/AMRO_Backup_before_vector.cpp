...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	while (true) {
		...
		for (int j = 0; j < nPoints; j++) {
			...
			output[3 * steps * j + 0 + 0] = starts[j * 3 + 0];
			output[3 * steps * j + 0 + 1] = starts[j * 3 + 1];
			output[3 * steps * j + 0 + 2] = starts[j * 3 + 2];
			
			
			for (int i = 1; i < steps; i++) {

				double argx0 = output[3 * steps * j + 3 * (i - 1) + 0];
				double argy0 = output[3 * steps * j + 3 * (i - 1) + 1];
				double argz0 = output[3 * steps * j + 3 * (i - 1) + 2];
